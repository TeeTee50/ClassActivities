...
const coords = data.map.apply(user => {
...
// Block
const lingoLonger = data.map(user => {
  let catchPhrase = user.company.catchPhrase;
  let bs = user.company.bs;
  return `${catchPhrase} ${bs}`;
});
console.log(lingoLonger);

// Showing a reason why you might have to use block arrow function instead of single line
const lingoUpper = data.map(user => {
  let catchPhrase = user.company.catchPhrase;
  let bs = user.company.bs;
  let combined = catchPhrase + bs;
  return combined.toUpperCase();
});
console.log(lingoUpper);
